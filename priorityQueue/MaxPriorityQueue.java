...
public class MaxPriorityQueue<T>{
	...
	public T removeMax() throws PriorityQueueException {
		...
		if((rightChildIndex < heap.size() /**<-- checks wheteher rightchildIndex within the range i.e rightChildIndex is null or not  otherwise will give error if we not add this conditon */) &&( heap.get(rightChildIndex).priority > heap.get(maxIndex).priority) ) {
